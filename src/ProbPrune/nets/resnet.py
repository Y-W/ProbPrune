...
class PreActBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, prune=None):
...
class PreActBottleneck(nn.Module):
    """Pre-activation version of the BasicBlock."""
    def __init__(self, in_channels, out_channels, stride=1, prune=None, bottleneck_factor=0.25):
...
class StageBlock(nn.Module):
    def __init__(self, num_block, in_channels, out_channels, stride=2, prune=None, bottleneck_factor=None):
        ...
        if bottleneck_factor is None:
            b = PreActBlock(in_channels, out_channels, stride, prune)
        else:
            b = PreActBottleneck(in_channels, out_channels, stride, prune, bottleneck_factor)
        self.num_prune.extend(b.num_prune)
        blocks.append(b)

        for i in xrange(num_block - 1):
            if bottleneck_factor is None:
                b = PreActBlock(out_channels, out_channels, 1, prune)
            else:
                b = PreActBottleneck(out_channels, out_channels, 1, prune, bottleneck_factor)
            self.num_prune.extend(b.num_prune)
            blocks.append(b)
...
class ResNet(nn.Module):
    def __init__(self, config, prune):
        ...
        if first_conv_type == 'cifar':
            last_channels = 16
            self.first_conv = nn.Conv2d(num_input, 16, kernel_size=3, padding=1, bias=False)

        else:
            last_channels = 64
            self.first_conv = nn.Conv2d(num_input, 64, kernel_size=7, padding=3, bias=False)

        if self.prune is not None:
            self.num_prune.append(last_channels)
        self.first_bn = nn.BatchNorm2d(last_channels)

        target_channels = st_channels
        stages = []
        for k in block_nums:
            stage = StageBlock(k, last_channels, target_channels, stride=2, prune=prune,
