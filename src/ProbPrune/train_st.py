...
from .ops_modules import binary_sigmoid_prune


class PruneST(nn.Module):
    def __init__(self, net, prune_coeff):
        super(PruneST, self).__init__()
        self.net = net
        self.prune_vec = nn.ParameterList([nn.Parameter(torch.zeros((k, 1, 1)))
                                           for k in self.net.num_prune])
